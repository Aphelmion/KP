/**
 * Created by Toshiba on 09.04.14.
 */
...
var EllipseCtrl = {
    ...
    addPoint: function (event) {
        ...
        if (this.getExistingPoints().length == 3) {
            ...
            var addEllipseCommand = new AddEllipseCommand(existingPoints[length - 3], existingPoints[length - 2], existingPoints[length - 1]);
            app.executeCommand(addEllipseCommand);
            this.clearPoints();
        }
    },
    ...
    createMacroCommand : function(pointCoordinates) {
        ...
        var addPointCommand = new AddPointCommand(point);
        var addEllipseCommand = new AddEllipseCommand(existingPoints[length - 2], existingPoints[length - 1], point);
        macroCommand.addCommands(addPointCommand,addEllipseCommand);
    ...
    addEllipsePoint: function (point) {
        this.points.push(point);
    },
    ...
    movePoint : function(event) {
        ...
        if (mouseDownCoordinates[0] != mouseUpCoordinates[0] && mouseDownCoordinates[1] != mouseUpCoordinates[1]){
            ...
            if (currentElement instanceof JXG.Curve){
                var moveEllipseCommand = new MoveEllipseCommand(event);
                app.executeCommand(moveEllipseCommand);
            }
